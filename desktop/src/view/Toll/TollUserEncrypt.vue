...
const queryEncrypt = () => {
  server.post('/tool/userEncrypt', { info: query.encrypt }, (data: BaseResult) => {
    query.decrypt = data.message
    query.copyText = data.message
  })
}
const queryDecrypt = () => {
  server.post('/tool/userDecrypt', { info: query.decrypt }, (data: BaseResult) => {
...
<template>
  <div>
    ...
    <main>
      <p class="tc">(注:此加密与解密为用户级接口，一但忘记账号将无法已加密的信息解密)</p>
